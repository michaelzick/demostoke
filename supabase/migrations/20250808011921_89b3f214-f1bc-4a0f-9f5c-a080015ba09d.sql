...
-- Drop the existing "Authenticated users can create demo events" policy if it exists
DROP POLICY IF EXISTS "Authenticated users can create demo events" ON public.demo_calendar;

-- Create a stricter INSERT policy for admins only
CREATE POLICY IF NOT EXISTS "Only admins can create demo events"
