...
-- Phase 1: Immediate Table Maintenance

-- Run VACUUM FULL on heavily bloated tables
VACUUM FULL public.app_settings;
VACUUM FULL public.blog_posts;
VACUUM FULL public.user_roles;
VACUUM FULL public.profiles;
VACUUM FULL public.equipment;

-- Phase 2: Advanced Indexing Strategy

-- Specialized composite indexes for complex queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_equipment_search_composite 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_equipment_user_category 
ON public.equipment(user_id, category, status);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_equipment_location_category 
ON public.equipment(category, location_lat, location_lng) 
WHERE location_lat IS NOT NULL AND location_lng IS NOT NULL;

-- Partial indexes for filtered queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_equipment_available_featured 
ON public.equipment(created_at DESC, view_count DESC) 
WHERE status = 'available' AND is_featured = true;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_equipment_visible_trending 
ON public.equipment(view_count DESC, name) 
WHERE visible_on_map = true AND status = 'available' AND view_count > 0;

-- Covering indexes for frequently accessed columns
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_equipment_list_covering 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_profiles_search_covering 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_equipment_text_search 
ON public.equipment USING gin(to_tsvector('english', name || ' ' || COALESCE(description, '')));

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_blog_posts_text_search 
ON public.blog_posts USING gin(to_tsvector('english', title || ' ' || excerpt || ' ' || content));

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_profiles_text_search 
ON public.profiles USING gin(to_tsvector('english', COALESCE(name, '') || ' ' || COALESCE(about, '')));

-- Array indexes for tags
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_blog_posts_tags 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_equipment_pagination_created 
ON public.equipment(created_at DESC, id) 
WHERE status = 'available' AND visible_on_map = true;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_equipment_pagination_price 
ON public.equipment(price_per_day ASC, id) 
WHERE status = 'available' AND visible_on_map = true;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_equipment_pagination_views 
ON public.equipment(view_count DESC, id) 
WHERE status = 'available' AND visible_on_map = true;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_blog_posts_pagination 
ON public.blog_posts(published_at DESC, id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_equipment_reviews_pagination 
ON public.equipment_reviews(created_at DESC, id);

-- Optimized indexes for JOIN operations
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_equipment_images_join 
ON public.equipment_images(equipment_id, display_order, is_primary);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_pricing_options_join 
ON public.pricing_options(equipment_id, duration);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_equipment_views_analytics 
...
AS $$
BEGIN
  REFRESH MATERIALIZED VIEW CONCURRENTLY mv_trending_equipment;
  REFRESH MATERIALIZED VIEW CONCURRENTLY mv_equipment_stats;
END;
$$;
