...
WITH CHECK (
  auth.uid() = id AND
  -- Prevent users from changing their ID (should never happen but extra safety)
  OLD.id = NEW.id AND
  -- Prevent users from changing creation timestamps
  OLD.created_at = NEW.created_at AND
  OLD.member_since = NEW.member_since
);
...
-- Users can only update their own equipment (non-critical fields)
...
WITH CHECK (
  ...
  -- Even admins cannot change creation timestamp
  OLD.created_at = NEW.created_at
);
