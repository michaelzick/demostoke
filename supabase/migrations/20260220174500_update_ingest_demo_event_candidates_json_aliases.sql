-- Expand manual JSON ingest normalization aliases for nested event payloads.
...
DECLARE
  ...
  v_start_local_text text;
  v_start_local_timestamp timestamp;
  v_category_hint_text text;
...
BEGIN
  ...
  LOOP
    ...
    v_title := NULLIF(BTRIM(COALESCE(v_row ->> 'title', v_row ->> 'event_title', v_row ->> 'event_name', v_row ->> 'name', '')), '');
    v_company := NULLIF(BTRIM(COALESCE(v_row ->> 'company', v_row ->> 'event_host', v_row ->> 'brand', v_row ->> 'host', v_row ->> 'organizer', '')), '');
    ...
    IF jsonb_typeof(v_row -> 'location') = 'object' THEN
      v_location := NULLIF(BTRIM(COALESCE(v_row #>> '{location,address}', v_row #>> '{location,name}', '')), '');
    ELSE
      v_location := NULLIF(BTRIM(COALESCE(v_row ->> 'location', v_row ->> 'venue', v_row ->> 'address', '')), '');
    END IF;

    IF v_location IS NULL THEN
      v_location := NULLIF(BTRIM(COALESCE(v_row ->> 'venue', v_row ->> 'address', '')), '');
    END IF;

    v_source_primary_url := NULLIF(BTRIM(COALESCE(
      v_row ->> 'source_primary_url',
      v_row ->> 'source_url',
      v_row ->> 'url',
      v_row #>> '{source,url}',
      v_row #>> '{source,primary_url}',
      ''
    )), '');
    ...
    v_location_lat_text := NULLIF(BTRIM(COALESCE(v_row ->> 'location_lat', v_row ->> 'lat', v_row #>> '{location,lat}', '')), '');
    v_location_lng_text := NULLIF(BTRIM(COALESCE(v_row ->> 'location_lng', v_row ->> 'lng', v_row #>> '{location,lng}', '')), '');

    IF v_event_date_text IS NULL THEN
      v_start_local_text := NULLIF(BTRIM(COALESCE(v_row #>> '{dates_times,0,start_local}', v_row #>> '{dates_times,0,start}', v_row #>> '{dates_times,0,start_at}', '')), '');
      IF v_start_local_text IS NOT NULL THEN
        BEGIN
          v_start_local_timestamp := REPLACE(v_start_local_text, ' ', 'T')::timestamp;
          v_event_date_text := TO_CHAR(v_start_local_timestamp::date, 'YYYY-MM-DD');
          IF v_event_time_text IS NULL THEN
            v_event_time_text := TO_CHAR(v_start_local_timestamp::time, 'HH24:MI:SS');
          END IF;
        EXCEPTION
          WHEN others THEN
            NULL;
    ...
    ELSIF v_gear_category_raw IN ('mountain-bikes', 'mountain bike', 'mountain bikes', 'mountain_bikes', 'mtb')
      OR v_gear_category_raw ~ '(mountain[ _-]?bike|mtb|bike park)' THEN
      v_gear_category := 'mountain-bikes';
    ELSIF v_gear_category_raw IN ('skis_snowboards', 'skis-snowboards', 'ski_snowboard', 'ski-snowboard') THEN
      v_category_hint_text := LOWER(COALESCE(v_title, '') || ' ' || COALESCE(v_company, '') || ' ' || COALESCE(v_equipment_available, ''));
      IF v_category_hint_text LIKE '%snowboard%' THEN
        v_gear_category := 'snowboards';
      ELSE
        v_gear_category := 'skis';
      END IF;
