...
- Cron-generated gear-review drafts use the simple user prompt `write a comprehensive review of the [gear brand and model]`. They must not include availability, booking details, location details, prices, dollar amounts, or rate structures in public copy. They must not use em dashes in public copy, must contain at least 2000 visible body characters after stripping HTML tags, and must store a selected gear image URL for the blog thumbnail instead of a small Google `thumbnailLink`.
