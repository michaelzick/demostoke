...
- Map popup builders return DOM elements populated with `textContent`; consumers must use Mapbox `setDOMContent()`. SSR structured data and hydration JSON escape `<`, and HTML string insertion uses replacement callbacks to preserve literal `$` sequences.
