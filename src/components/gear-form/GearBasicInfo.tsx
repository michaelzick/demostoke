...
import { generateGearDescription, rewriteGearDescription } from "@/services/equipment/descriptionAIService";
import {
  Select,
  SelectContent,
  SelectItem,
...
type DescriptionType = 'user' | 'generated' | 'rewritten';
...
const GearBasicInfo = ({
  ...
  const [descriptionType, setDescriptionType] = useState<DescriptionType>('user');
  ...
  const handleRewriteDescription = async () => {
    if (!description.trim()) {
      toast({
        ...
        description: "Please paste some text in the description field first.",
        variant: "destructive",
      });
      return;
    }
    ...
    setIsRewriting(true);
    const rewritten = await rewriteGearDescription(description, gearName, gearType);
    setIsRewriting(false);

    if (rewritten) {
      setDescription(rewritten);
      setDescriptionType('rewritten');
    } else {
      toast({
        title: "Error",
        description: "Failed to rewrite description.",
        variant: "destructive",
      });
    }
  };
      ...
      <div>
        ...
        <div className="flex items-center gap-2 mt-2">
          ...
          <Button
            ...
            disabled={isRewriting || isGenerating || !description.trim()}
            size="sm"
          >
            {isRewriting ? (
              <>
