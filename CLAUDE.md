...
- `index.html` contains the pre-hydration theme resolver plus a consent-gated analytics bootstrap. GTM, GA4, and Amplitude scripts are injected only by `window.__loadAnalytics()` after the visitor opts in via the cookie consent banner. If you touch head behavior, inspect this file.
...
- All analytics (GTM, GA4, Amplitude incl. session replay) are consent-gated and opt-in for every visitor. Scripts load only via `window.__loadAnalytics()` in the `index.html` bootstrap; consent state lives in `src/utils/cookieConsent.ts` (`cookie-consent` key, versioned via `CONSENT_VERSION`, localStorage + cookie mirror, GPC honored). The banner is `src/components/CookieConsentBanner.tsx` (mounted via `ClientOnlyCookieConsent` in `App.tsx`; reopened from the Footer's Cookie Settings button). `trackEvent`, `GoogleTagManager`, and `vitals.sendToGA4` all guard on `hasAnalyticsConsent()` so pre-consent events never enter the dataLayer queue (GTM replays it on late load). Never add analytics scripts directly to `index.html` or push to `dataLayer` without a consent check. The `cookie-consent` key/version literals are duplicated between `index.html` and `cookieConsent.ts` — keep them in sync.
